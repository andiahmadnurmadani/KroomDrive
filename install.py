...
def write_pm2_config(backend_port):
    """Production: only backend runs under PM2. Frontend is served as static by Express."""
    cfg = {
        "apps": [
            {
                "name":          "kroomdrive",
    ...
    eco_path = ROOT / "ecosystem.config.json"
    eco_path.write_text(json.dumps(cfg, indent=2))
    ok(f"PM2 ecosystem config: {eco_path.name}")
    return eco_path
...
def write_dev_scripts(frontend_port, backend_port):
    """Simple dev-mode launch scripts (no PM2) — runs Vite dev server + backend."""
    if SYSTEM == "Windows":
...
sleep 1
cd "$ROOT" && npm run dev &
echo "KroomDrive dev running — http://localhost:{frontend_port}"
...
def write_pm2_scripts():
    """Scripts that use PM2 (production mode)."""
    if SYSTEM == "Windows":
        (ROOT / "start.bat").write_text(
            "@echo off\necho Starting KroomDrive via PM2...\n"
            "pm2 start ecosystem.config.json\npm2 save\n"
            "echo Done. Use: pm2 logs kroomdrive\n"
        )
...
def write_nginx_config(domain, backend_port):
    """Generate nginx config with proper WebSocket proxy for KroomDrive.
    
    Architecture: nginx → backend (port) which serves both:
      - /api, /socket.io  → Express API + Socket.IO
      - /*                → React SPA (static dist/)
    """
    has_domain = domain and domain not in ("localhost", "127.0.0.1", "")
    
    ssl_block = ""
    if has_domain:
        ssl_block = f"""
    # SSL — managed by Certbot (run: certbot --nginx -d {domain})
    # listen 443 ssl;
    ...
    server_name = domain if has_domain else "_"
    
    config = f"""# KroomDrive nginx config — generated by install.py
# Place this file at: /etc/nginx/sites-available/kroomdrive
# Enable: ln -s /etc/nginx/sites-available/kroomdrive /etc/nginx/sites-enabled/
# Test:   nginx -t
# Reload: systemctl reload nginx

upstream kroomdrive_backend {{
...
{ssl_block}
    ...
    # Proxy all requests to KroomDrive backend
    # (Express serves both API and static frontend from the same port)
    location / {{
        proxy_pass         http://kroomdrive_backend;
        ...
        # Timeouts — generous for large file transfers
...
"""
    nginx_path = ROOT / "kroomdrive.nginx.conf"
    nginx_path.write_text(config)
    ok(f"Nginx config → {nginx_path.name}")
    return nginx_path
...
def write_env_files(frontend_port, backend_port, admin_user,
    ...
    fe_env.write_text(
        ...
        f"# VITE_PORT and VITE_BACKEND_URL are used in dev mode only\n"
    ...
    be_env.write_text(
        ...
        f"# CORS — with nginx reverse proxy, use * (nginx handles SSL/origin)\n"
...
def main():
    ...
    # ── Step 6: Domain (for nginx config) ─────────────────────────────────────
    step(6, "Domain / nginx config")
    if IS_TTY and deps["nginx"].found:
        print(f"  {gray('Enter your domain to generate a ready-to-use nginx config.')}")
        print(f"  {gray('Leave blank to skip nginx config generation.')}")
        print()
        domain = ask("Domain name (e.g. drive.kroombox.com)", "")
    else:
        domain = ""
        if not deps["nginx"].found:
            info("nginx not found — skipping nginx config")
        else:
            info("Non-interactive mode — skipping nginx config")

    # ── Step 7: Security keys ─────────────────────────────────────────────────
    step(7, "Generating security keys")
    jwt_secret = secrets.token_hex(64)
    ok(f"JWT secret generated (128-char hex)")

    # ── Step 8: Install npm packages ──────────────────────────────────────────
    step(8, "Installing npm packages")
    npm_install(ROOT, "Frontend")
    npm_install(BACKEND_DIR, "Backend")

    # ── Step 9: Build frontend ────────────────────────────────────────────────
    step(9, "Building frontend")
    info("Running: npm run build  (this may take a minute…)")
    ...
    if build_result.returncode != 0:
        ...
        print(build_result.stderr[-800:])
        sys.exit(1)
    ok("Frontend built → dist/")

    # ── Step 10: Directories & env files ─────────────────────────────────────
    step(10, "Setting up directories and config files")
    ...
    nginx_path = None
    if domain or (deps["nginx"].found and IS_TTY):
        step(11, "Generating nginx config")
        nginx_path = write_nginx_config(domain, be_port)

    # ── Step 12: PM2 / scripts ────────────────────────────────────────────────
    step(12, "Creating launch scripts")
    if use_pm2:
        eco = write_pm2_config(be_port)
        write_pm2_scripts()
    else:
...
""")
    ...
    if started:
        access_url = f"https://{domain}" if domain else f"http://localhost:{be_port}"
        print(f"  {bold('KroomDrive is running:')}")
        print(f"  {cyan(access_url)}")
    else:
    ...
    if nginx_path:
        print(f"""
  {bold('nginx setup:')}
    {cyan(f'sudo cp {nginx_path} /etc/nginx/sites-available/kroomdrive')}
    {cyan('sudo ln -s /etc/nginx/sites-available/kroomdrive /etc/nginx/sites-enabled/')}
    {cyan('sudo nginx -t && sudo systemctl reload nginx')}
    {gray('# For HTTPS (recommended):')}
    {cyan(f'sudo certbot --nginx -d {domain}') if domain else gray('  # set your domain first')}""")
  ...
  {bold('Useful PM2 commands:')}
    ...
    {cyan('pm2 logs kroomdrive')}     — app logs only
    {cyan('pm2 restart kroomdrive')}  — restart
    {cyan('pm2 stop kroomdrive')}     — stop
    {cyan('./restart.sh')}            — quick restart

  {bold('Architecture (production):')}
    {gray('nginx → :'+str(be_port)+' (Express serves API + static frontend)')}
    {gray('No separate frontend process needed!')}
