...
export const UpdateBadge: React.FC<UpdateBadgeProps> = ({ onClick, checking, hasUpdate, collapsed }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-2 px-3 py-2.5 rounded-xl bg-amber-50 border border-amber-200 text-amber-700 hover:bg-amber-100 transition-colors text-xs font-bold ${
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in slide-in-from-bottom-4 duration-200">
        ...
        {/* Log output (visible during/after update) */}
        {showLogs && (
          <div className="border-b border-gray-100">
            ...
            >
              ...
              Update log ({logs.length} lines)
            </button>
            {showLogs && (
              <div className="bg-gray-950 px-4 py-3 max-h-60 overflow-y-auto font-mono text-[11px] leading-relaxed">
                {logs.map((l, i) => (
                  <div key={i} className={`${
            ...
            <div className="space-y-4">
              <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 text-sm text-amber-700">
                ...
                <ol className="list-decimal list-inside space-y-0.5 text-amber-600 text-xs">
                  <li>Stash local changes (if any)</li>
                  <li>git pull (latest code)</li>
                  <li>npm install (frontend + backend)</li>
                  <li>vite build</li>
                  <li>pm2 restart kroomdrive</li>
                </ol>
                <p className="mt-2 text-xs text-amber-500">App briefly unavailable during restart (~30s).</p>
              </div>
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3 bg-red-50 border border-red-100 rounded-xl p-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-red-400 mt-2">Check the update log above for details.</p>
                </div>
              </div>
              <div className="flex gap-3">
                <button onClick={onClose} className="flex-1 px-4 py-2.5 text-sm text-gray-600 border border-gray-200 hover:bg-gray-50 rounded-xl transition-colors font-medium">
